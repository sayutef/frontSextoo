...
export default function Profile() {
  ...
  // ---------------------------------------------------------------------
  // 🔥 MODO OSCURO PERSISTENTE (igual que en Camera y Grafic)
  // ---------------------------------------------------------------------
  ...
  const handleUpdateUserName = () => {
    ...
    serviceChangeUser(
      ...
      (error) => alert('Error al actualizar usuario: ' + error)
    );
  };

  return (
    <div className="flex">
      {/* Menú con modo oscuro */}
      <Menu darkMode={darkMode} toggleDarkMode={toggleDarkMode} />

      <div
        className={`
          flex-1 min-h-screen p-6 space-y-8 transition-colors duration-300
          ${darkMode ? 'bg-gray-900 text-white' : 'bg-[#faf9f2] text-gray-900'}
        `}
      >
        {!user ? (
          <div className="flex justify-center items-center h-[calc(100vh-1.5rem)] overflow-hidden">
            <img
              src={robotNot}
              alt="Cargando perfil"
              className="max-w-md w-full h-100 object-contain"
            />
          </div>
        ) : (
          <>
            ...
            <div
              className={`
                rounded-xl p-6 shadow-md flex items-center justify-between transition-colors
                ${darkMode ? 'bg-gray-800 text-white' : 'bg-teal-500 text-white'}
              `}
            >
            ...
            <div
              className={`
                rounded-xl p-6 shadow-md transition-colors
            ...
            <div
              className={`
                rounded-xl p-6 shadow-md flex flex-col lg:flex-row justify-between items-center gap-8 transition-colors
                ${darkMode ? 'bg-gray-800 text-white' : 'bg-white text-gray-900'}
              `}
            >
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className={`
                        ...
                        ${darkMode
                          ? 'bg-gray-700 text-white border-gray-600 focus:ring-teal-400'
